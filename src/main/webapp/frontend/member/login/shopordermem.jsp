...
.table th, .table td {
	...
	padding: 8px; /* 增加內距  */
 	text-align: center !important; /* 左對齊文本  */ 
...
.table th {
	...
	<div class="col-12 col-md-8">
		<!--             <div class="card"> -->
		<!--                 <div class="card-body"> -->
		<h3 class="center-table text-center p-2">商城訂單</h3>
			...
			<tbody>
				<c:forEach items="${all}" var="order">
					<tr>
						...
						<fmt:formatDate value="${order.prodOrderDate}"
						...
						<td><c:choose>
								<c:when test="${order.orderStatus == 0}">
									<p>
										<span class="text-success">成立</span>
									</p>
								</c:when>
								<c:when test="${order.orderStatus == 1}">
									<p>
										<span class="text-primary">出貨</span>
									</p>
								</c:when>
								<c:when test="${order.orderStatus == 2}">
									<p>
										<span class="text-warning">完成</span>
									</p>
								</c:when>
						...
						<td><a
							href="shoporderdetailmem.jsp?shopOrderNo=${order.shopOrderNo}"
	...
	<script
		src="${pageContext.request.contextPath}/frontend/member/js/groupOrder.js"></script>
</body>
</html>
