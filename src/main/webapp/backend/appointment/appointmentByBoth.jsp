...
<%-- 	<p>currentPage != PTAFPageQty5 = ${currentPage != PTAFPageQty5}</p> --%>
	...
	<c:if test="${currentPage > 1}">
		<a href="${pageContext.request.contextPath}/privatetrainingappointmentform?action=getboth&page=1&trainerNo=${param.trainerNo}&memName=${param.memName}">至第一頁</a>&nbsp;
	</c:if>
	<c:if test="${currentPage - 1 != 0}">
		<a href="${pageContext.request.contextPath}/privatetrainingappointmentform?action=getboth&page=${currentPage - 1}&trainerNo=${param.trainerNo}&memName=${param.memName}">上一頁</a>&nbsp;
	</c:if>
	<c:if test="${currentPage + 1 <= PTAFPageQty5}">
		<a href="${pageContext.request.contextPath}/privatetrainingappointmentform?action=getboth&page=${currentPage + 1}&trainerNo=${param.trainerNo}&memName=${param.memName}">下一頁</a>&nbsp;
	</c:if>
	<c:if test="${currentPage == PTAFPageQty5}">
		<a href="${pageContext.request.contextPath}/privatetrainingappointmentform?action=getboth&page=${PTAFPageQty5}&trainerNo=${param.trainerNo}&memName=${param.memName}">至最後一頁</a>&nbsp;
	</c:if>
