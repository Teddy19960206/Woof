...
public class ClassOrderDAOImpl implements ClassOrderDAO{
	...
	public List<ClassOrder> getAll() {
		return getSession().createQuery("FROM ClassOrder ORDER BY coTime DESC", ClassOrder.class).list();
	}

	@Override
	public List<ClassOrder> getAll(int currentPage) {
		int first = (currentPage - 1) * PAGE_MAX_RESULT;
		return getSession().createQuery("FROM ClassOrder ORDER BY coTime DESC", ClassOrder.class)
	...
	public List<ClassOrder> getByMemNo(String memNo, int currentPage) {
		int first = (currentPage - 1) * PAGE_MAX_RESULT;
		return getSession().createQuery("FROM ClassOrder c WHERE c.member.memNo LIKE :memNo ORDER BY coTime DESC", ClassOrder.class)
	...
	public List<ClassOrder> getOrderByMemNo(String memNo) {
		return getSession().createQuery("FROM ClassOrder c WHERE c.member.memNo = :memNo ORDER BY coTime DESC", ClassOrder.class)
				.setParameter("memNo", memNo)
				.list();
	}
